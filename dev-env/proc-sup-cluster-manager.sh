...
# Procure Supplies Cluster Manager
# Manages the core proc-sup cluster with chaos engineering capabilities
...
CLUSTER_FILES="proc-sup-store-volumes.yaml,ex-esdb-network.yaml,proc-sup-store-cluster.yaml"
CLUSTER_PROJECT="proc-sup"
CLUSTER_DESCRIPTION="3 nodes (proc-sup-0,1,2)"
...
print_header() {
  clear
  print_color $CYAN "╔══════════════════════════════════════════════════════════════╗"
  print_color $CYAN "║                 'Procure Supplies' Cluster                   ║"
  print_color $CYAN "║                                                              ║"
...
get_cluster_status() {
  local filter_pattern="name=^proc-sup-[0-2]$"
...
show_status() {
  ...
  printf "  %-12s %s %s\\n" "proc-sup" "$(get_cluster_status)" "($CLUSTER_DESCRIPTION)"
  ...
  if [[ -d "/volume/proc-sup-store" ]]; then
    local size=$(du -sh /volume/proc-sup-store 2>/dev/null | cut -f1)
    echo -e "  /volume/proc-sup-store    ${GREEN}●${NC} Available ($size)"
  else
    echo -e "  /volume/proc-sup-store    ${RED}●${NC} Not found"
  fi
}

# Function to start the cluster
start_cluster() {
  print_color $YELLOW "Starting Procure Supplies cluster ($CLUSTER_DESCRIPTION)..."
  echo
  ...
  print_color $CYAN "Creating data directories for proc-sup store..."
  sudo mkdir -p /volume/proc-sup-store/{data0,data1,data2}
  ...
  if [[ $? -eq 0 ]]; then
    print_color $GREEN "✓ Procure Supplies cluster started successfully!"
  else
    print_color $RED "✗ Failed to start Procure Supplies cluster"
  fi
}

# Function to stop the cluster
stop_cluster() {
  print_color $YELLOW "Stopping Procure Supplies cluster..."
  echo
  ...
  if [[ $? -eq 0 ]]; then
    print_color $GREEN "✓ Procure Supplies cluster stopped successfully!"
  else
    print_color $RED "✗ Failed to stop Procure Supplies cluster"
  fi
}
...
show_logs() {
  print_color $YELLOW "Showing logs for Procure Supplies cluster..."
  echo
...
show_resource_usage() {
  ...
  docker ps --format "table {{.Names}}\\t{{.Status}}\\t{{.Ports}}" | grep -E "(NAMES|proc-sup-)"
  echo

  # Show resource consumption
  print_color $CYAN "Resource Consumption:"
  docker stats --no-stream --format "table {{.Container}}\\t{{.CPUPerc}}\\t{{.MemUsage}}" | grep -E "(CONTAINER|proc-sup-)"
  echo
...
get_random_running_node() {
  local running_nodes
  running_nodes=($(docker ps --filter "name=^proc-sup-" --format "{{.Names}}" 2>/dev/null))
...
unpause_all_nodes() {
  ...
  local paused_nodes
  paused_nodes=($(docker ps --filter "status=paused" --filter "name=^proc-sup-" --format "{{.Names}}" 2>/dev/null))

defmodule ExESDB.ConsistencyChecker do
  ...
  defp detect_potential_issues(members, leader_info, cross_node_check, _raft_status) do
    []
    |> check_leader_presence(leader_info)
    |> check_node_communication(cross_node_check)
    |> check_membership_consistency(members, cross_node_check)
  end

  defp check_leader_presence(issues, leader_info) do
    if Map.get(leader_info, :leader) == nil do
      [:no_leader | issues]
    else
      issues
    end
  ...
  defp check_node_communication(issues, cross_node_check) do
  ...
  defp check_membership_consistency(issues, members, cross_node_check) do
    ...
    if length(inconsistent_views) > 0 do
